#############################################################################
# Makefile for building: server.app/Contents/MacOS/server
# Generated by qmake (3.1) (Qt 6.7.0)
# Project:  server.pro
# Template: app
# Command: /opt/homebrew/bin/qmake -o Makefile server.pro
#############################################################################
...
DISTDIR = /Volumes/BMP/DBSP/Projects/server/.tmp/server1.0.0
...
LIBS          = $(SUBLIBS) -F/opt/homebrew/lib -framework QtGui -framework AppKit -framework ImageIO -framework Metal -framework QtNetwork -framework QtCore -framework IOKit -framework DiskArbitration -framework AGL -framework OpenGL   
...
EXPORT__QMAKE_STASH_ = /Volumes/BMP/DBSP/Projects/server/.qmake.stash
...
EXPORT__PRO_FILE_ = /Volumes/BMP/DBSP/Projects/server/server.pro
...
server.app/Contents/MacOS/server:  $(OBJECTS)  
	@test -d server.app/Contents/MacOS/ || mkdir -p server.app/Contents/MacOS/
	$(LINK) $(LFLAGS) -o $(TARGET)  $(OBJECTS) $(OBJCOMP) $(LIBS)

Makefile: server.pro /opt/homebrew/share/qt/mkspecs/macx-clang/qmake.conf /opt/homebrew/share/qt/mkspecs/features/spec_pre.prf \
		/opt/homebrew/share/qt/mkspecs/features/device_config.prf \
		/opt/homebrew/Cellar/qt/6.7.0_1/share/qt/mkspecs/common/unix.conf \
		/opt/homebrew/Cellar/qt/6.7.0_1/share/qt/mkspecs/common/mac.conf \
		/opt/homebrew/Cellar/qt/6.7.0_1/share/qt/mkspecs/common/macx.conf \
...
server.app/Contents/PkgInfo: 
	@test -d server.app/Contents || mkdir -p server.app/Contents
	@$(DEL_FILE) server.app/Contents/PkgInfo
	@echo "APPL????" > server.app/Contents/PkgInfo
server.app/Contents/Info.plist: 
	@test -d server.app/Contents || mkdir -p server.app/Contents
	@$(DEL_FILE) server.app/Contents/Info.plist
	@plutil -convert xml1 -o - /opt/homebrew/share/qt/mkspecs/macx-clang/Info.plist.app | sed -e "s,@SHORT_VERSION@,1.0,g" -e "s,\$${QMAKE_SHORT_VERSION},1.0,g" -e "s,@FULL_VERSION@,1.0.0,g" -e "s,\$${QMAKE_FULL_VERSION},1.0.0,g" -e "s,@TYPEINFO@,????,g" -e "s,\$${QMAKE_PKGINFO_TYPEINFO},????,g" -e "s,@BUNDLEIDENTIFIER@,com.yourcompany.server,g" -e "s,\$${PRODUCT_BUNDLE_IDENTIFIER},com.yourcompany.server,g" -e "s,\$${MACOSX_DEPLOYMENT_TARGET},13.0,g" -e "s,\$${IPHONEOS_DEPLOYMENT_TARGET},,g" -e "s,\$${TVOS_DEPLOYMENT_TARGET},,g" -e "s,\$${WATCHOS_DEPLOYMENT_TARGET},,g" -e "s,\$${IOS_LAUNCH_SCREEN},LaunchScreen,g" -e "s,@ICON@,,g" -e "s,\$${ASSETCATALOG_COMPILER_APPICON_NAME},,g" -e "s,@EXECUTABLE@,server,g" -e "s,@LIBRARY@,server,g" -e "s,\$${EXECUTABLE_NAME},server,g" -e "s,@TYPEINFO@,????,g" -e "s,\$${QMAKE_PKGINFO_TYPEINFO},????,g" >server.app/Contents/Info.plist

all: Makefile \
		server.app/Contents/PkgInfo \
		server.app/Contents/Info.plist server.app/Contents/MacOS/server
